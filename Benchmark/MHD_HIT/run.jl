# Benchmark/MHD_HIT/run.jl — Forced 3D MHD turbulence (stationary HIT-MHD)
# Usage: cd Benchmark/MHD_HIT && julia run.jl [nsteps]
#
# Forced MHD turbulence driven to a statistical steady state by HIT linear
# forcing (Lundgren 2003 / Rosales & Meneveau 2005), applied to BOTH the
# velocity and the magnetic field. Initial condition is a Passot-Pouquet
# divergence-free random field; the linear forcing then sustains the cascade.
#
# In-situ energy spectrum E_kin(k) / E_mag(k) is written to SPEC/ periodically.
#
# Domain: [0, 2π]³, triply periodic, γ = 5/3 (ideal MHD, inviscid)
# Mesh:   generate first with  julia gen_mesh.jl [Nx]   (default 64³)
#
# Tunable parameters: hit_forcing_A (forcing amplitude), u_rms_init, B_rms_init, k0_init
...
const flow_forcing::Bool = false       # HIT forcing triggered by test_case, not forcing_mode
const forcing_mode::Int64 = 0          # (unused for HIT; kept for solver compat)
...
const hit_forcing_A::FT = FT(0.1)       # HIT linear forcing amplitude (ε = 2A·E)
...
# ─── Mesh (relative to Benchmark/MHD_HIT/) ───
...
# ─── Flow control ───
const test_case::String = "MHDHIT"    # triggers HIT forcing on both v and B
...
# ─── Output (relative to Benchmark/MHD_HIT/) ───
...
#
# For a forced MHD-HIT run, the linear forcing injects energy at rate 2A·E; the
# total energy grows until numerical dissipation balances injection (statistical
# steady state). The divergence of B should stay small (~1e-2) if GLM is working.
# Energy spectrum E_kin(k)/E_mag(k) is written to SPEC/ every _spectrum_interval steps.

const _stats_file = joinpath(@__DIR__, "stats.dat")
const _stats_interval = 20
const _spectrum_interval = 200    # FFT is expensive; do it less often than stats

# Write header once on rank 0
function _init_stats_file()
    if MPI.Comm_rank(MPI.COMM_WORLD) == 0
        open(_stats_file, "w") do io
            println(io, "# Forced MHD-HIT turbulence diagnostics (linear forcing A=$(Main.hit_forcing_A))")
...
function in_situ_post_process(tt, activeTime, current_dt, blocks, world_rank, Block_Nprocs, block_comms)
    ...
    # Energy spectrum E_kin(k) / E_mag(k) — written to SPEC/ every _spectrum_interval
    if tt % _spectrum_interval == 0
        write_energy_spectrum(tt, activeTime, blocks, world_rank, MPI.COMM_WORLD;
                              out_dir=@__DIR__, Lx=Lx, Ly=Lx, Lz=Lx)
    end
end
...
if rank == 0
    println("=" ^ 70)
    println("  Flame3D — Forced MHD-HIT Turbulence (linear forcing on v and B)")
    println("=" ^ 70)

# Generate the two-block Cartesian control mesh for the MPI Orszag-Tang test.
...
@inline cartesian_point(xi::FT, eta::FT, zeta::FT) = (xi, eta, zeta)
...
function write_block(path, nx, ny, nz, eta_lo, eta_hi)
    ...
    for k in eachindex(zetas), j in eachindex(etas), i in eachindex(xis)
        point = cartesian_point(xis[i], etas[j], zetas[k])
...
function generate_mesh(nx::Integer, ny::Integer, nz::Integer, out_dir::AbstractString)
    ...
    open(joinpath(out_dir, "mesh_manifest.toml"), "w") do io
        println(io, "mesh_kind = \"two_block_periodic_cartesian\"")
    ...
    println("Generated Cartesian two-block mesh: $out_dir")
    return out_dir
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    out_dir = length(ARGS) >= 4 ? ARGS[4] : joinpath(@__DIR__, "mesh", "cartesian_N32")

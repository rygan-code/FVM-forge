...
@inline function _ct_mhd_characteristic_reconstruct_kernel!(
    ...
    i_extension = DIRECTION == 1 ? Int32(0) : Int32(1)
    j_extension = DIRECTION == 2 ? Int32(0) : Int32(1)
    k_extension = DIRECTION == 3 ? Int32(0) : Int32(1)
    if i > nxp+NG+i_extension || j > nyp+NG+j_extension ||
       k > nzp+NG+k_extension || i < NG || j < NG || k < NG
        return
    end
    ...
    @inbounds begin
        nx = nx_array[face_i, face_j, face_k]
        ny = ny_array[face_i, face_j, face_k]
        nz = nz_array[face_i, face_j, face_k]
        area = area_array[face_i, face_j, face_k]
        face_bn = face_b[face_i, face_j, face_k] / area
    end
...
@inline function _ct_mhd_characteristic_weno7_cache_kernel!(
    Q, cache, area_array, nx_array, ny_array, nz_array, face_b, Vol,
...
function Eigen_reconstruct_i(Q, U, ϕ, S, Fx, Areai, nxi, nyi, nzi, nxp, nyp, nzp,
    ...
    # 1. Boundary check - CT mode extends j/k by 1 for ghost face-EMF
    @static if ct_mode
        if i > nxp+NG || j > nyp+NG+Int32(1) || k > nzp+NG+Int32(1) || i < NG || j < NG || k < NG
            return
        end
    else
...
function Conser_reconstruct_i(Q, U, ϕ, S, Fx, rho_sum_x, Areai, nxi, nyi, nzi, nxp, nyp, nzp,
    ...
    @static if ct_mode
        @static if @isdefined(ct_emf_scheme) &&
            ...
            if i > nxp+NG || j > nyp+NG+Int32(1) ||
               k > nzp+NG+Int32(1) || i < NG || j < NG || k < NG
                return
            end
        end
    else
    ...
    if _use_primitive_reconstruction
        if eigen_reconstruction
            ...
            @inbounds _bn_plm = Bx_face_CT[_bi_plm, _bj_plm, _bk_plm] / Area
    ...
    @static if strict_ct_positivity && ct_mode
        if splitMethodID == Int32(4)
            @inbounds _Bn_recon = Bx_face_CT[_bi, _bj, _bk] / Area
    ...
    @static if ct_mode
        @inbounds _Bn_face = Bx_face_CT[_bi, _bj, _bk] / Area
    ...
    @static if ct_mode
        ...
        if j >= NG && j <= nyp+NG+Int32(1) &&
           k >= NG && k <= nzp+NG+Int32(1)
            @inbounds rho_sum_x[i-NG+1, j-NG+1, k-NG+1] = UL_vec[1] + UR_vec[1]
            @inbounds for n = 1:Ncons
                Fx[i-NG+1, j-NG+1, k-NG+1, n] = flux_temp[n] * Area
            end
        end
    else
        @inbounds for n = 1:Ncons
...
function Conser_reconstruct_j(Q, U, ϕ, S, Fy, rho_sum_y, Areaj, nxj, nyj, nzj, nxp, nyp, nzp,
    ...
    if _use_primitive_reconstruction
        if eigen_reconstruction
            ...
            @inbounds _bn_plm = By_face_CT[_bi_plm, _bj_plm, _bk_plm] / Area
    ...
    @static if strict_ct_positivity && ct_mode
        if splitMethodID == Int32(4)
            @inbounds _Bn_recon = By_face_CT[_bi, _bj, _bk] / Area
            _WL_reconstructed = SVector{Ncons,FT}(
                ntuple(n -> @inbounds(UL_final[n]), Val(Ncons))::NTuple{Ncons,FT},
            )
            _WR_reconstructed = SVector{Ncons,FT}(
    ...
    @static if ct_mode
        @inbounds _Bn_face = By_face_CT[_bi, _bj, _bk] / Area
        _UL_before_face_b = UL_vec
        _UR_before_face_b = UR_vec
        UL_vec = ct_impose_face_bn_preserve_p(UL_vec, nx, ny, nz, _Bn_face)
        UR_vec = ct_impose_face_bn_preserve_p(UR_vec, nx, ny, nz, _Bn_face)
    ...
    @static if ct_mode
        ...
        if i >= NG && i <= nxp+NG+Int32(1) &&
           k >= NG && k <= nzp+NG+Int32(1)
            @inbounds rho_sum_y[i-NG+1, j-NG+1, k-NG+1] = UL_vec[1] + UR_vec[1]
            @inbounds for n = 1:Ncons
                Fy[i-NG+1, j-NG+1, k-NG+1, n] = flux_temp[n] * Area
            end
        end
    else
        @inbounds for n = 1:Ncons
...
function Conser_reconstruct_k(Q, U, ϕ, S, Fz, rho_sum_z, Areak, nxk, nyk, nzk, nxp, nyp, nzp,
    ...
    @static if ct_mode
        @static if @isdefined(ct_emf_scheme) &&
            ...
            if i > nxp+NG+Int32(1) || j > nyp+NG+Int32(1) ||
               k > nzp+NG || i < NG || j < NG || k < NG
                return
            end
        end
    else
    ...
    if _use_primitive_reconstruction
        if eigen_reconstruction
            ...
            @inbounds _bn_plm = Bz_face_CT[_bi_plm, _bj_plm, _bk_plm] / Area
    ...
    @static if strict_ct_positivity && ct_mode
        if splitMethodID == Int32(4)
            @inbounds _Bn_recon = Bz_face_CT[_bi, _bj, _bk] / Area
            _WL_reconstructed = SVector{Ncons,FT}(
                ntuple(n -> @inbounds(UL_final[n]), Val(Ncons))::NTuple{Ncons,FT},
            )
            _WR_reconstructed = SVector{Ncons,FT}(
    ...
    @static if ct_mode
        @inbounds _Bn_face = Bz_face_CT[_bi, _bj, _bk] / Area
        _UL_before_face_b = UL_vec
        _UR_before_face_b = UR_vec
        UL_vec = ct_impose_face_bn_preserve_p(UL_vec, nx, ny, nz, _Bn_face)
        UR_vec = ct_impose_face_bn_preserve_p(UR_vec, nx, ny, nz, _Bn_face)
    ...
    @static if ct_mode
        ...
        if i >= NG && i <= nxp+NG+Int32(1) &&
           j >= NG && j <= nyp+NG+Int32(1)
            @inbounds rho_sum_z[i-NG+1, j-NG+1, k-NG+1] = UL_vec[1] + UR_vec[1]
            @inbounds for n = 1:Ncons
                Fz[i-NG+1, j-NG+1, k-NG+1, n] = flux_temp[n] * Area
            end
        end
    else
        @inbounds for n = 1:Ncons

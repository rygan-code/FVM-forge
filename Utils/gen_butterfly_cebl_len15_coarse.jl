...
# Coarse grid: 512 x 54 x 54 (half resolution in all x, y, z directions)
# =============================================================================

# Include the base mesh generator
include("gen_butterfly_fvm.jl")

function main_cebl_len15_coarse()
    Lx_override = 15.0
    scale = Lx_override / Lx  # 15.0 / 7.5 = 2.0

    println("=== Extended 15.0m Pipe Mesh Generator (COARSE) ===")
    println("  Lx = $(Lx_override) ($(Lx_override/R0)R₀)")
    println("  Scale factor:  $(scale)")

    # ── Grid Resolution (Halved) ──
    out_dir = "MESH_LEN15_COARSE"
    Nx_base = 256
    Ny_b0, Nz_b0, N_rad = 54, 54, 54

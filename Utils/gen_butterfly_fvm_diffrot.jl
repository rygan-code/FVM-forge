# =============================================================================
# Butterfly (O-H) Mesh Generator for Differential Rotation Pipe Flow
# Extended domain: Lx = 20R₀ (15R physical + 5R fringe)
# Self-contained script — includes gen_butterfly_fvm.jl for reusing functions.
# =============================================================================

# Include the base mesh generator (functions only, main() guarded)
include("gen_butterfly_fvm.jl")

function main_diffrot()
    Lx_override = 10.0  # 20R₀
    scale = Lx_override / Lx  # 10.0 / 7.5

    println("=== Differential Rotation Pipe Mesh ===")
    println("  Lx = $(Lx_override) ($(Lx_override/R0)R₀)")
    println("  Physical zone: 0 → $(0.75*Lx_override) ($(0.75*Lx_override/R0)R₀)")
    println("  Fringe zone:   $(0.75*Lx_override) → $(Lx_override) ($(0.25*Lx_override/R0)R₀)")
    println("  Scale factor:  $(scale)")

    # ── COARSE mesh ──
    out_dir = "MESH_DIFFROT_COARSE"
    Nx_base = 512
    Ny_b0, Nz_b0, N_rad = 108, 108, 108

    # Scale Nx proportionally, round to multiple of 8

...
@inline function mhd_raw_thermo_components(
    ...
    magnetic = half * (magnetic_x^2 + magnetic_y^2 + magnetic_z^2)
    internal = energy - kinetic - magnetic
    pressure = (gamma - one(gamma)) * internal
...
@inline function ct_impose_face_b_preserve_p(
    ...
    bold = U[BI]
    energy = U[5] + T(0.5) * (bface * bface - bold * bold)
...
@inline function ct_impose_face_bn_preserve_p(
    ...
    energy = U[5] + T(0.5) * (new_magnetic2 - old_magnetic2)
    return SVector{N,T}(
        ntuple(Val(N)) do n
            n == 5 ? energy :
            n == 6 ? magnetic_x :
    ...
    for first_index in 1:5
        ...
        for second_index in 1:5
            ...
            @inbounds begin
                ...
                flux_point += weight * face_flux[ii, jj, kk]
...
@inline function ct_recover_cell_b_point6(
    ...
    i_faces = ntuple(Val(6)) do face
        _ct_face_point_data6(
            Bx_face, Areai, nxi, nyi, nzi,
            i + face - 3, j, k, Val(1),
        )
    end
    j_faces = ntuple(Val(6)) do face
        _ct_face_point_data6(
            By_face, Areaj, nxj, nyj, nzj,
            i, j + face - 3, k, Val(2),
        )
    end
    k_faces = ntuple(Val(6)) do face
        _ct_face_point_data6(
            Bz_face, Areak, nxk, nyk, nzk,
            i, j, k + face - 3, Val(3),
        )
    end
    T = eltype(Bx_face)
...
@inline function ct_mhd_point_conservative_to_primitive(
    ...
    magnetic_energy = T(0.5)*dot(magnetic, magnetic)
    pressure = (gamma-one(T))*(hydro[5]-kinetic-magnetic_energy)
    return SVector{9,T}(
...
@inline function ct_mhd_primitive_to_characteristic(basis, delta::SVector{7,T}) where {T}
    ...
    magnetic_beta = basis.beta2*delta[6] + basis.beta3*delta[7]
    ...
    return SVector{7,T}(
        ...
        T(0.5)*(
            basis.beta2*(delta[7]*basis.bsign*basis.isqrtd + delta[4]) -
            basis.beta3*(delta[6]*basis.bsign*basis.isqrtd + delta[3])
        ),
...
@inline function ct_primitive_to_conservative(
    ...
    energy = pressure / (gamma - one(T)) + kinetic + magnetic
    return SVector{9,T}(
        rho,
        rho * velocity_x,
        rho * velocity_y,

...
- Magnetic energy (E_mag = integral( B^2/(2*mu0) dV )) (only if MHD)
"""
function compute_integral_energies(blocks, comm_world)
    ...
    for (bid, b) in blocks
        ...
        @static if equation_type == :MHD
            ...
            # Magnetic energy density is B^2/(2*mu0), integrated using 1/Vol.
            inv_mu0 = Float64(INV_MU0_SI)
            emag_local += mapreduce((bx, by, bz, v) ->
                0.5 * inv_mu0 * (Float64(bx)^2 + Float64(by)^2 +
                                  Float64(bz)^2) / Float64(v), +,
                Bx_v, By_v, Bz_v, vol_v)
        end
    end
...
function checkerboard_diagnostic!(blocks, connectivity, tt_val, world_rank)
    ...
    for ((dst_bid, fid), conn) in connectivity
        ...
        b = blocks[dst_bid]
        src_fid = conn.src_f
        is_cross_axes = structured_face_normal_axis(src_fid) !=
                        structured_face_normal_axis(fid)
        
        is_cross = is_cross_axes
        ...
        if fid == 1 || fid == 2
            for layer in 1:4
                ...
                for jc in (ng+1):(Ny+ng), kc in (ng+1):(Nz+ng)
                    if ic > 1 && ic < size(U_cpu, 1)
                        d = U_cpu[ic, jc, kc, n] -
                            0.5 * (U_cpu[ic-1, jc, kc, n] + U_cpu[ic+1, jc, kc, n])
                        e_2dx += d^2
                        count += 1
                    end
                end
    ...
    for ((dst_bid, fid), conn) in connectivity
        ...
        src_fid = conn.src_f
        is_cross = structured_face_normal_axis(src_fid) !=
                   structured_face_normal_axis(fid)

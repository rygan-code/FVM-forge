...
# The explicit magnetic source changes total energy by B·dB/dt / mu0.
# CT keeps its existing compact-source contract here and does not inject a
# volume source directly into face-centered B.
function HIT_forcing_kernel_MHD!(dU_forced, Q, A_force,
    ...
    magnetic_work = @static if ct_mode
        zero(FT)
    else
        mhd_magnetic_forcing_work(Bx, By, Bz, fBx, fBy, fBz)
    end
    @inbounds dU_forced[i, j, k, 5] =
        u * fx + v * fy + w * fz + magnetic_work
    # GLM stores magnetic forcing in U[6:8]. CT advances face-B through the
    # edge EMF update, so its compact source buffer has no magnetic slots.
    @static if !ct_mode
...
# Uses an exact source substep: ψ^{n+1} = ψ^n · exp(-dt · ch / cr).
# The time integrator owns the Strang half-step scheduling.
# where ch = ch_glm_current (auto-computed), cr = cr_glm (user-set)
function glm_source_kernel!(U, Q, dt, ch, cr, nxp, nyp, nzp)
    i = (blockIdx().x - Int32(1)) * blockDim().x + threadIdx().x
    j = (blockIdx().y - Int32(1)) * blockDim().y + threadIdx().y
    k = (blockIdx().z - Int32(1)) * blockDim().z + threadIdx().z
...
function compute_cf_max_kernel!(cf_arr, Q, nxp, nyp, nzp)
    ...
    va2 = B2 * INV_MU0_SI / ρ # Alfvén speed²
